...
#fd = nltk.FreqDist([(vs) for mot in texte_sans_sw for vs in re.findall(r'\b([a-z]*xy)|(\b[a-z]*wy) | (\b[a-z]*ky)', mot)])
...
#//TODO la structure de freqdist change s'il y a des recherches de regexp : s'adapter à liste de tuples (string, int) et (tuple, int)
data_terme = []
data_occu = []
va_terme = ""
for i in mc100:
    print(type(i[0]))
    if isinstance(type(i[0]),tuple):
        print("isinstance tuple")   # TODO from here !!
        print (type(i[0]))
        print(i[0][0])
        print(type(i[0][0]))
        for j in i[0][0]:
